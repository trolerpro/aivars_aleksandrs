...
def submit():
    if request.method == 'POST':
        ...
        if prieksmets:
            ...
            conn.execute(
                "INSERT INTO preiksmeti (prieksmets) VALUES (?)",
                (prieksmets,)
            )
            conn.commit()
            conn.close()
        ...
        if prieksmets and atzime:
            try:
                ...
                if 0 <= atzime <= 10:
                    conn = sqlite3.connect(DB_PATH)
                    conn.execute(
                        "INSERT INTO videjas (prieksmets, atzime) VALUES (?, ?)",
                        (prieksmets, atzime)
                    )
                    conn.commit()
                    conn.close()
...
def create_task():
    ...
    cursor = conn.cursor()
    cursor.execute("""
        INSERT INTO tasks (title, description, due_date, priority, status, progress)
        VALUES (?, ?, ?, ?, ?, ?)
    """, (
        data['title'],
...
def update_task(task_id):
    ...
    cursor.execute("""
        UPDATE tasks
        SET title=?, description=?, due_date=?, priority=?, status=?, progress=?, updated_at=CURRENT_TIMESTAMP
        WHERE id=?
    """, (
        data['title'],
        data.get('description', ''),
